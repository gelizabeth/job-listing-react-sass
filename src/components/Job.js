    ...
    return (
        ...
        <div className={`job ${job.featured ? 'job_featured': ''}`}>
            ...
            <div className="job__tags">
                ...
                {job.languages?.map((language, index) => <div key={index} className="job__tag" value={language} onClick={selectFilter}>{language}</div>)}
                {job.tools?.map((tool, index )=> <div key={index} className="job__tag" value={tool} onClick={selectFilter}>{tool}</div>)}
            </div>
        </div>
    )
}
